...
const ContactForm = ({ prefilledProduct }: ContactFormProps) => {
  ...
  const validate = () => {
    ...
    if (!form.products.trim())
      errs.products = "Please mention the products";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 🔥 EmailJS send
      await emailjs.send(
        "service_4y75y6p", // replace
        "template_bjkl7rp", // replace
        {
          name: form.name,
          email: form.email,
          mobile: form.mobile,
        ...
        "sPe_qrSiT57LJMubR" // replace
      );

      // Optional: store locally
      const orders = JSON.parse(
        localStorage.getItem("glamgrip_orders") || "[]"
      );
      const order = {
        ...form,
  ...
  if (status === "success") {
    return (
      <motion.div
        ...
        className="bg-card rounded-2xl p-8 text-center border border-border/30 shadow-lg"
      >
        <CheckCircle
        ...
        <button
          ...
          className="mt-6 font-body text-sm text-cta font-semibold underline"
        >
          Place another order
        </button>
      </motion.div>
  ...
  return (
    <form
      ...
      className="bg-card rounded-2xl p-6 sm:p-8 border border-border/30 shadow-lg space-y-4"
    >
      ...
      <div>
        ...
        <textarea
          value={form.products}
          onChange={(e) =>
            handleChange("products", e.target.value)
          }
          rows={3}
